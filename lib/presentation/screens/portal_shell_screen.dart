...
class _ResortPortalShellState extends ConsumerState<ResortPortalShell> {
  ...
  Widget build(BuildContext context) {
    ...
    final mainScaffold = Scaffold(
      ...
      bottomNavigationBar: (!isDesktop && activeRole == UserRole.customer)
          ? BottomNavigationBar(
              currentIndex: activeTab == 'profile' ? 3 : activeTab == 'trips' ? 2 : activeTab == 'saved' ? 1 : 0,
              onTap: (index) {
                ref.read(activeTabProvider.notifier).state = ['explore', 'saved', 'trips', 'profile'][index];
              },
              items: const [
      ...
      body: Stack(
        children: [
          Column(
            children: [
              if (!isDesktop)
                _buildMobileHeader(context, ref, activeRole, authenticatedRole, unreadNotifs, effectiveTopPadding),
              Expanded(
          ...
          if (!isDesktop && _isNotifOpen)
            Positioned(
              ...
              right: 16,
              child: _buildNotificationDropdown(notifications, unreadNotifs, ref),
            ),
        ],
      ),
    );
  ...
  void _openWhatsApp() async {
    ...
    final uri = Uri.parse('whatsapp://send?phone=919876543210&text=Hello%20VSP%20Nest%20Concierge,%20I%20need%20assistance%20with%20my%20stay.');
    final webUri = Uri.parse('https://wa.me/919876543210?text=Hello%20VSP%20Nest%20Concierge,%20I%20need%20assistance%20with%20my%20stay.');
    try {
      if (await canLaunchUrl(uri)) { await launchUrl(uri); }
      else { await launchUrl(webUri, mode: LaunchMode.externalApplication); }
    } catch (_) {
      try { await launchUrl(webUri, mode: LaunchMode.externalApplication); }
      catch (_) { if (context.mounted) SnackbarHelper.error(context, 'Could not launch WhatsApp.'); }
    }
  }

  // ── Desktop Sidebar ──

  Widget _buildDesktopSidebar(WidgetRef ref, UserRole activeRole, UserRole? authenticatedRole, String activeTab) {
    return Container(
      width: 290,
        ...
        children: [
          ...
          AppCard.glass(
            ...
            child: Column(
              ...
              children: [
                Text('ACTIVE PERSPECTIVE', style: AppTextStyles.labelSm.copyWith(color: AppColors.goldAccent.withValues(alpha: 0.8))),
                const SizedBox(height: AppSpacing.sm),
                Row(
                  children: [
                    Container(width: 8, height: 8, decoration: const BoxDecoration(color: AppColors.goldAccent, shape: BoxShape.circle)),
                    const SizedBox(width: AppSpacing.sm),
                    Text(_getRoleLabel(activeRole), style: GoogleFonts.inter(color: AppColors.white, fontSize: 13.5, fontWeight: FontWeight.bold)),
                  ],
                ),
                if (authenticatedRole != null && authenticatedRole != activeRole) ...[
                  ...
                  Row(
                    children: [
                      Icon(Icons.info_outline, size: 10, color: AppColors.goldAccent.withValues(alpha: 0.6)),
                      const SizedBox(width: AppSpacing.xs),
                      Text('Logged in as ${_getRoleLabel(authenticatedRole)}',
                          style: GoogleFonts.inter(color: AppColors.goldAccent.withValues(alpha: 0.6), fontSize: 9.5, fontWeight: FontWeight.w500)),
                    ],
                  ),
                ],
              ],
              ...
              children: [
                if (authenticatedRole == UserRole.superAdmin) ...[
                  Padding(
                    ...
                    child: Text('SIMULATE SYSTEM ROLES', style: AppTextStyles.labelSm.copyWith(color: Colors.white38)),
                  ),
                ...
                if (activeRole == UserRole.customer) ...[
                  ...
                  Padding(
                    padding: const EdgeInsets.only(left: 8, bottom: 8),
                    child: Text('GUEST SUITE CHANNELS', style: AppTextStyles.labelSm.copyWith(color: Colors.white38)),
                  ),
  ...
  void _switchRole(WidgetRef ref, UserRole role, UserRole? authenticatedRole) async {
    if (role == authenticatedRole) {
      ref.read(activeRoleProvider.notifier).state = role;
      if (role == UserRole.customer) ref.read(activeTabProvider.notifier).state = 'villa';
      if (context.mounted) Navigator.pushReplacementNamed(context, AppRouter.routeForRole(role));
      return;
    }
    final confirmed = await AppDialog.confirm(
      context: context,
      title: 'Switch Perspective',
      message: 'Switching from ${_getRoleLabel(authenticatedRole ?? role)} to ${_getRoleLabel(role)}.\n\n'
  ...
  Widget _sidebarRoleItem(WidgetRef ref, String title, UserRole role, IconData icon, UserRole activeRole, UserRole? authenticatedRole) {
    ...
    return Container(
      ...
      decoration: isSelected ? BoxDecoration(borderRadius: AppRadius.mdBr, boxShadow: AppShadows.glowGold) : null,
      child: Material(
        color: AppColors.mossGreen,
        borderRadius: AppRadius.mdBr,
        clipBehavior: Clip.antiAlias,
        child: Ink(
          decoration: isSelected ? const BoxDecoration(gradient: AppGradients.gold) : null,
          child: ListTile(
            leading: Icon(icon, color: isSelected ? AppColors.mossGreen : Colors.white60, size: 20),
            title: Text(title,
    ...
    return Container(
      ...
      child: Material(
        ...
        child: Ink(
          decoration: isSelected ? const BoxDecoration(gradient: AppGradients.gold) : null,
          child: ListTile(
            leading: Icon(icon, color: isSelected ? AppColors.mossGreen : Colors.white54, size: 18),
            title: Text(title,
                style: GoogleFonts.inter(
                    fontSize: 11.5,
                    fontWeight: isSelected ? FontWeight.bold : FontWeight.w500,
      ...
      child: Material(
        ...
        child: ListTile(
          ...
          title: Text('Logout Perspective',
              style: GoogleFonts.inter(fontSize: 12.5, fontWeight: FontWeight.w500, color: Colors.white70)),
          onTap: () {
      ...
      children: [
        ...
        Padding(
          ...
          child: Text(
            ...
            style: AppTextStyles.labelSm.copyWith(fontSize: 8.5, color: Colors.white24),
          ),
        ),
      ],
    );
  ...
  Widget _buildMobileHeader(
    ...
    return Container(
      decoration: BoxDecoration(
        ...
        border: const Border(bottom: BorderSide(color: AppColors.lightBone, width: 1)),
        boxShadow: [BoxShadow(color: AppColors.mossGreen.withValues(alpha: 0.04), blurRadius: 10, offset: const Offset(0, 4))],
      ),
      padding: EdgeInsets.only(left: 16, right: 16, top: effectiveTopPadding + 14, bottom: 14),
      child: Row(
        children: [
          Container(
            decoration: BoxDecoration(
              shape: BoxShape.circle,
              boxShadow: [BoxShadow(color: AppColors.goldAccent.withValues(alpha: 0.2), blurRadius: 6, spreadRadius: 1)],
            ),
              ...
              children: [
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(color: AppColors.goldAccent.withValues(alpha: 0.5), width: 1),
                  ),
                  child: Text('PORTAL', style: AppTextStyles.labelSm.copyWith(fontSize: 8, color: AppColors.goldAccent)),
                ),
              ],
            ),
          ),
          Row(
            ...
            children: [
              _mobileIconButton(
                ...
                badge: unreadNotifs.isNotEmpty ? '${unreadNotifs.length}' : null,
  ...
  Widget _mobileIconButton({required IconData icon, String? badge, required VoidCallback onTap}) {
    return Container(
      height: 32, width: 32,
      decoration: BoxDecoration(
        ...
        children: [
          Material(
            ...
            child: InkWell(
              ...
              onTap: onTap,
              child: Center(child: Icon(icon, color: AppColors.mossGreen, size: 16)),
            ),
          ),
          if (badge != null)
            Positioned(
              ...
              child: Container(
                ...
                child: Center(
                  child: Text(badge,
                      style: AppTextStyles.labelSm.copyWith(fontSize: 7, color: AppColors.white)),
                ),
              ),
            ),
        ],
  ...
  Widget _mobileRoleSwitcher(WidgetRef ref, UserRole authenticatedRole) {
    return Container(
      ...
      child: PopupMenuButton<UserRole>(
        icon: const Icon(Icons.swap_horiz_rounded, color: AppColors.mossGreen, size: 18),
        ...
        itemBuilder: (context) => [
          _roleMenuItem(UserRole.customer, Icons.hotel_outlined, 'Customer Space'),
          _roleMenuItem(UserRole.admin, Icons.dashboard_customize_outlined, 'Admin Desk'),
          _roleMenuItem(UserRole.staff, Icons.cleaning_services_outlined, 'Staff Ops'),
          _roleMenuItem(UserRole.accountant, Icons.receipt_long_outlined, 'Accountant Ledger'),
          _roleMenuItem(UserRole.superAdmin, Icons.admin_panel_settings_outlined, 'Super Admin Config'),
        ],
      ),
    );
  }

  PopupMenuItem<UserRole> _roleMenuItem(UserRole role, IconData icon, String label) {
  ...
  Widget _buildActiveViewport(UserRole role, String activeTab) {
    switch (role) {
      case UserRole.customer:
        switch (activeTab) {
          case 'explore':
          case 'villa': return const CustomerView();
          case 'calendar': return const ValleyCalendarView();
          case 'saved': return const SavedView();
          case 'trips':
          case 'dashboard': return const CustomerDashboardView();
          case 'profile': return const CustomerProfileView();
          default: return const CustomerView();
        }
      case UserRole.admin: return const AdminView();
      case UserRole.staff: return const StaffView();
      case UserRole.accountant: return const AccountantView();
      case UserRole.superAdmin: return const SuperAdminView();
    }
  }

  // ── Notifications ──

  Widget _buildNotificationDropdown(List<AppNotification> notifications, List<AppNotification> unreadNotifs, WidgetRef ref) {
    return ClipRRect(
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
                ...
                child: Row(
                  ...
                  children: [
                    Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text('Operational Logs &\nAlerts', style: AppTextStyles.titleSm.copyWith(fontSize: 16)),
                        const SizedBox(height: 4),
                        Text('Sandbox cross-role updates', style: AppTextStyles.bodyXs),
                      ],
                    ),
                    if (unreadNotifs.isNotEmpty)
                      TextButton(
                        ...
                        child: Text('Dismiss Admin\nAlerts',
                            ...
                            style: GoogleFonts.inter(fontSize: 11, fontWeight: FontWeight.bold, color: AppColors.mossGreen)),
                      ),
                  ],
                ),
              ),
                ...
                child: notifications.isEmpty
                    ? const Padding(
                        ...
                        child: AppEmptyState(
                          ...
                          subtitle: 'Trigger actions in other roles to populate.',
                        ),
                      )
                    : ListView.separated(
                        shrinkWrap: true,
                        ...
                        itemBuilder: (context, index) {
                          ...
                          return Material(
                            color: notif.read ? Colors.transparent : AppColors.goldAccent.withValues(alpha: 0.08),
                            child: InkWell(
                              ...
                              child: Container(
                                decoration: BoxDecoration(
                                  border: notif.read
                                      ? null
                                      : const Border(left: BorderSide(color: AppColors.goldAccent, width: 3)),
                                ),
                                padding: const EdgeInsets.all(12),
                                child: Row(
                                  crossAxisAlignment: CrossAxisAlignment.start,
                                      ...
                                      child: Column(
                                        ...
                                        children: [
                                          Row(
                                            mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                            children: [
                                              Expanded(child: Text(notif.title, style: GoogleFonts.inter(fontSize: 12, fontWeight: FontWeight.bold, color: AppColors.mossGreen))),
                                              Text(_formatTime(notif.timestamp), style: AppTextStyles.bodyXs),
                                            ],
                                          ),
                                          const SizedBox(height: 4),
                                          Text(notif.message, style: GoogleFonts.inter(fontSize: 11, color: AppColors.charcoal.withValues(alpha: 0.7), height: 1.4)),
                                        ],
                                      ),
                                    ),
                                  ],
              ...
              Padding(
                padding: const EdgeInsets.all(12),
                child: Text('Notifications sync dynamically across role dashboards',
                    textAlign: TextAlign.center, style: GoogleFonts.inter(fontSize: 10, color: AppColors.charcoal.withValues(alpha: 0.4))),
              ),
            ],
          ),
        ),
